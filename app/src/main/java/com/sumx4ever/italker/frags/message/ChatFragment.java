...
import com.sumx4ever.common.tools.AudioPlayHelper;
import com.sumx4ever.common.widget.PortraitView;
import com.sumx4ever.common.widget.adapter.TextWatcherAdapter;
...
import com.sumx4ever.italker.factory.utils.FileCache;
import com.sumx4ever.italker.frags.panel.PanelFragment;
...
public abstract class ChatFragment<InitModel>
    ...
    // 给界面的Appbar设置一个舰艇，得到关闭与打开的时候的进度
    ...
    // 输入框监听
    @OnTextChanged(value = R.id.edit_content, callback = OnTextChanged.Callback.AFTER_TEXT_CHANGED)
    void onContentChange(Editable s) {
        String content = s.toString().trim();
        boolean needSendMsg = !TextUtils.isEmpty(content);
        // 设置状态，改变对应的Icon
        mSubmit.setActivated(needSendMsg);
    ...
    void onSubmitClick() {
        if (mSubmit.isActivated()) {
            // 发送
            String contnt = mContent.getText().toString();
            mContent.setText("");
            mPresenter.pushText(contnt);
        } else {
            onMoreClick();
        }
    }
    ...
    public void onAdapterDataChanged() {
        if (!mRecyclerView.hasNestedScrollingParent(ViewCompat.TYPE_NON_TOUCH)) {
            mRecyclerView.startNestedScroll(View.SCROLL_AXIS_VERTICAL, ViewCompat.TYPE_NON_TOUCH);
        }
        ...
        protected void onBind(Message message) {
            ...
            if (mLoading != null) {
                ...
                if (status == Message.STATUS_DONE) {
                    ...
                    mLoading.setForegroundColor(UiCompat.getColor(getResources(), R.color
                            .colorAccent));
                    mLoading.start();
