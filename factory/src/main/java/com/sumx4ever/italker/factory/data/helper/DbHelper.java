...
public class DbHelper {
     ...
     * 观察者集合
     * Class<?> 观察者的表
     * Set<ChangedListener> 一个表对应的观察者有很多
     */
    private final Map<Class<?>, Set<ChangedListener>> changedListeners = new HashMap<>();

    /**
     * 从所有监听者中，获取某一个表的所有监听者
     *
     * @param tClass  表对应的Class信息
     * @param <Model> 范型
     * @return Set<ChangedListener>
     */
    private <Model extends BaseModel> Set<ChangedListener> getListeners(final Class<Model> tClass) {
     ...
     */
    public static <Model extends BaseModel> void save(final Class<Model> tClass, final Model...
            models) {
     ...
     * @param <Model> 这个实例的范型，限定条件是BaseModel
     */
    public static <Model extends BaseModel> void delete(final Class<Model> tClass, final Model...
            models) {
        if (models == null || models.length == 0)
            return;
        // 当前数据库的管理者
     ...
     * @param <Model> 这个实例的范型，限定条件是BaseModel
     */
    private final <Model extends BaseModel> void notifyDelete(final Class<Model> tClass, final
    Model...
            models) {
        ...
        definition.beginTransactionAsync(new ITransaction() {
            ...
            public void execute(DatabaseWrapper databaseWrapper) {
                ...
                for (Session.Identify identify : identifies) {
                    Session session = SessionHelp.findFromLocal(identify.id);
