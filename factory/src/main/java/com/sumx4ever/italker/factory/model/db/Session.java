...
import com.sumx4ever.italker.factory.data.helper.MessageHelper;
...
public class Session extends BaseDbModel<Session> {
    @PrimaryKey
    public String id; // Id, 是Message中的接收者User的Id或者群的Id
    @Column
    public String picture; // 图片，接收者用户的头像，或者群的图片
    @Column
    public String title; // 标题，用户的名称，或者群的名称
    @Column
    public String content; // 显示在界面上的简单内容，是Message的一个描述
    @Column
    public int receiverType = Message.RECEIVER_TYPE_NONE; // 类型，对应人，或者群消息
    @Column
    public int unReadCount; // 未读数量，当没有在当前界面时，应当增加未读数量
    @Column
    public Date modifyAt; // 最后更改时间

    @ForeignKey(tableClass = Message.class)
    public Message message; // 对应的消息，外键为Message的Id
    ...
    public boolean isSame(Session old) {
        return Objects.equals(id, old.id)
                && Objects.equals(receiverType, old.receiverType);
    }

    @Override
    public boolean isUiContentSame(Session old) {
        return this.content.equals(old.content)
